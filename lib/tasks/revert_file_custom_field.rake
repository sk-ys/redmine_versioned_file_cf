...
namespace :versioned_file_cf do
  desc 'Replace an existing Versioned file custom field with File and delete revision history'
  task revert_file_custom_field: :environment do
    ...
    I18n.with_locale(locale) do
      ...
      if dry_run
        puts I18n.t(
          'versioned_file_cf.tasks.revert_file_custom_field.label_dry_run',
          count: result.deleted_revisions_count
        )
      elsif result.deleted_revisions_count.zero?
        puts I18n.t('versioned_file_cf.tasks.revert_file_custom_field.label_done_no_values')
      else
        puts I18n.t(
  ...
  rescue VersionedFileCf::VersionedFileCustomFieldReverter::MigrationError => e
    abort e.message
  end
end