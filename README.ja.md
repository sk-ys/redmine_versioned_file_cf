...
このプラグインは `ファイル（リビジョン管理）` という新しいカスタムフィールド形式を追加し、アップロードされたファイルの履歴管理を可能にします。

英語版: [README.md](README.md)

## 機能

- 以下のオブジェクトへの `ファイル（リビジョン管理）` カスタムフィールドの挿入に対応
  - `Issue`, `Project`, `TimeEntry`, `Version`, `Document`, `User`, `Group`
- 既存のファイルカスタムフィールドと同様なファイルのアップロードに対応
- ファイル変更履歴の表示・削除・復元・差分表示に対応
- ファイルへの説明の追加および編集に対応
- 以下の権限設定に対応
  - 履歴の閲覧
  - 説明の編集
...
- アップロード対象のファイルがテキストファイルではない場合、差分表示はできません
...
- 既存のファイルカスタムフィールドに不整合な添付ファイルが含まれる場合は、変更を加えずに移行を中止します。
